import datetime
from typing import List, Optional, Union, Any, Awaitable, cast, Dict, TypeVar, Callable, Sequence
from uuid import UUID, uuid4
from functools import wraps
from datetime import datetime, timedelta
from sqlalchemy import text, and_, or_, exists, select, Boolean, DateTime, true, false, case, update, inspect
...
# JWT settings
JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", "your-secure-secret-key-123")  # Use environment variable
JWT_ALGORITHM = "HS256"
...
async def get_user_roles(db: Session, user_id: int) -> List[str]:
    ...
    try:
        ...
        for role in user_roles:
            role_name = db.scalar(select(Role.role_name).where(Role.role_id == role.role_id))
            if role_name:
...
async def has_permission(current_user: User, permission_feature: str, db: Optional[Session] = None) -> bool:
    ...
    try:
        ...
        if metadata:
            ...
            metadata = UserAuthMetadata(
                user_id=db.scalar(select(User.user_id).where(User.uuid == user_uuid)),
                provider=provider,
                external_user_id=str(db.scalar(select(User.uuid).where(User.uuid == user_uuid))),
                access_token=access_token,
                refresh_token=refresh_token,
                token_expires_at=token_expires_at,
...
def role_required(required_roles: List[str]):
    def decorator(func: AsyncCallable[T]) -> AsyncCallable[T]:
        @wraps(func)
        async def wrapper(*args: Any, current_user: User = Depends(get_current_user), db: Session = Depends(get_db), **kwargs: Any) -> T:
            # Get user roles using select()
            stmt = select(UserRole).join(Role).where(
                and_(
                    UserRole.user_id == current_user.user_id,
...
def admin_required(func: AsyncCallable[T]) -> AsyncCallable[T]:
    @wraps(func)
    async def wrapper(*args: Any, current_user: User = Depends(get_current_user), db: Session = Depends(get_db), **kwargs: Any) -> T:
