...
from db.models import (
    WorkOrder, WorkOrderAssignment, User, UserActivityLog, 
    WorkOrderStatusLog
)
...
async def assign_work_order(
    ...
    try:
        ...
        if not getattr(assignment, 'work_order_id', None) and not getattr(assignment, 'wo_number', None):
            raise HTTPException(status_code=422, detail="Either work_order_id or wo_number must be provided.")
        ...
        if not work_order:
            raise HTTPException(status_code=404, detail="Work order not found or inactive")
        work_order_status = db.scalar(
            select(WorkOrder.status).where(WorkOrder.work_order_id == work_order.work_order_id)
        )
        # Only allow assignment if status is PENDING or NEW
        if work_order_status not in [WorkOrderStatus.PENDING, "NEW"]:
            raise HTTPException(status_code=400, detail="Work order must be in PENDING or NEW status to assign")
        # Validate agent
        stmt = select(User).where(
            and_(
                User.user_id == assignment.agent_id,
        ...
        active_count = db.scalar(
            select(func.count()).select_from(WorkOrderAssignment)
            .join(WorkOrder, WorkOrderAssignment.work_order_id == WorkOrder.work_order_id)
            .where(
                and_(
        ...
        if active_count is None:
            active_count = 0
        if active_count >= 5:
            raise HTTPException(status_code=400, detail="Agent already has 5 active (IN_PROGRESS) work orders")
        ...
        if existing_assignment:
            raise HTTPException(status_code=400, detail="Work order already has an active assignment")
        # Create assignment
...
async def reassign_work_order(
    ...
    try:
        ...
        if not current_assignment:
            ...
            stmt = select(WorkOrderAssignment).where(
                and_(
                    WorkOrderAssignment.work_order_id == assignment_id,  # fallback: assignment_id is work_order_id
        ...
        if not current_assignment:
            raise HTTPException(status_code=404, detail="No assignment found to reassign")
        
        # Get work order
        work_order = db.query(WorkOrder).options(
            joinedload(WorkOrder.assignments),
            joinedload(WorkOrder.status_logs)
        ).filter(
        ...
        stmt = select(User).where(
            and_(
                User.user_id == reassignment.agent_id,
                User.activated.is_(True)
            )
        )
        agent = db.execute(stmt).scalar_one_or_none()
        ...
        current_agent_id = db.scalar(
            select(WorkOrderAssignment.agent_id)
            .where(WorkOrderAssignment.assignment_id == current_assignment.assignment_id)
        )
        
        # Prevent reassignment to same agent
        if current_agent_id == reassignment.agent_id:
            raise HTTPException(status_code=400, detail="Cannot reassign to the same agent")
...
async def acknowledge_work_order(
    ...
    try:
        ...
        if status_value not in valid_statuses:
            raise HTTPException(status_code=422, detail="Status must be either 'ACCEPTED' or 'REJECTED'.")
        if status_value == "REJECTED" and (not acknowledgment.remarks or not acknowledgment.remarks.strip()):
            raise HTTPException(status_code=422, detail="Remarks are required when status is 'REJECTED'.")
        ...
        if not assignment:
            raise HTTPException(status_code=404, detail="No active assignment found for this work order")
        ...
        if agent_id_value is None:
            raise HTTPException(status_code=500, detail="Assignment agent_id is not a valid value")
        if not isinstance(agent_id_value, int):
            raise HTTPException(status_code=500, detail="Assignment agent_id is not an integer value")
...
async def get_work_order(
    ...
    try:
        ...
        status_logs = db.query(WorkOrderStatusLog).filter(
            WorkOrderStatusLog.work_order_id == work_order_id
        ).all()
...
@router.post(
    "/start",
    response_model=WorkOrderStartResponse,
    summary="Start a work order",
    description="Starts a work order, updating its status to IN_PROGRESS and logging the change.",
    status_code=status.HTTP_200_OK
)
async def start_work_order(
    request: Request,
    response: Response,
    payload: WorkOrderStartRequest,
    force: bool = False,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    Start a work order (set status to IN_PROGRESS).
    """
    try:
        ...
        if not work_order:
            raise HTTPException(status_code=404, detail="Work order not found or inactive")
        status_val = getattr(work_order, 'status', None)
        if status_val not in [WorkOrderStatus.PENDING, "PENDING"]:
            raise HTTPException(status_code=400, detail="Work order must be in PENDING status")
        # Authorization
        user_id_val = getattr(current_user, 'user_id', None)
        if user_id_val is None or not isinstance(user_id_val, (int, float)):
            raise HTTPException(status_code=500, detail="Invalid user_id on current_user")
        user_roles = await get_user_roles(db, int(user_id_val))
        is_admin = "admin" in user_roles
        is_supervisor = "supervisor" in user_roles
        created_by_val = getattr(work_order, 'created_by', None)
        if created_by_val is None or not isinstance(created_by_val, (int, float)):
            raise HTTPException(status_code=500, detail="Invalid created_by on work_order")
        is_creator = (int(created_by_val) == int(user_id_val))
        # Only the assigned agent can start the work order
        is_assigned_agent = db.query(WorkOrderAssignment).filter(
            WorkOrderAssignment.work_order_id == work_order_id,
            WorkOrderAssignment.agent_id == int(user_id_val),
            WorkOrderAssignment.active.is_(True)
        ).count() > 0
        if not is_assigned_agent:
            raise HTTPException(status_code=403, detail="You are not assigned to this work order")
        # Optimistic lock: check if already started
        if str(work_order.status) == "IN_PROGRESS":
            raise HTTPException(status_code=409, detail="Work order already started by another user")
        ...
        # Log status change
        work_order_id_val = getattr(work_order, 'work_order_id', None)
        wo_number_val = getattr(work_order, 'wo_number', None)
        if work_order_id_val is None or wo_number_val is None:
            raise HTTPException(status_code=500, detail="Invalid work_order_id or wo_number on work_order")
        status_log = WorkOrderStatusLog(
            work_order_id=int(work_order_id_val),
            status="IN_PROGRESS",
            changed_by=int(user_id_val),
            changed_at=now,
        ...
        log = UserActivityLog(
            ...
            actor_id=int(user_id_val),
            action="start_work_order",
            details=f"Started work order {str(wo_number_val)}",
            timestamp=now
        )
        db.add(log)
        db.commit()
        # Refresh work_order to get updated values
        db.refresh(work_order)
        # Set Location header
        response.headers["Location"] = f"/work-orders/{int(work_order_id_val)}"
        updated_at_val = getattr(work_order, 'updated_at', now)
        return WorkOrderStartResponse(
            work_order_id=int(work_order_id_val),
            wo_number=str(wo_number_val),
            status="IN_PROGRESS",
